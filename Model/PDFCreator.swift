...
class PDFCreator: NSObject {
  ...
  func createPDFReport() -> Data {
    // 1
    let pdfMetaData = [
      kCGPDFContextCreator: "Firebltz Alarm Report",
      kCGPDFContextAuthor: "Fireblitz",
...
//      drawContactLabels(context, pageRect: pageRect, numberTabs: 8)
  ...
  func addBodyText(pageRect: CGRect, textTop: CGFloat) {
    // 1
    let textFont = UIFont.systemFont(ofSize: 7.0, weight: .regular)
    // 2
    ...
    // 4
    let textRect = CGRect(x: 10, y: textTop + 30, width: pageRect.width - 20,
  ...
  func addImage(pageRect: CGRect, imageTop: CGFloat) -> CGFloat {
    ...
    let imageRect = CGRect(x: imageX, y: 20,
...
class PDFCoverCreator: NSObject {
  let title: String
  let body: String
  let image: UIImage
  ...
  func createPDFReport() -> Data {
    // 1
    let pdfMetaData = [
      kCGPDFContextCreator: "Firehawk Alarm Report",
      kCGPDFContextAuthor: "Firehawk",
      kCGPDFContextTitle: title
    ]
    let format = UIGraphicsPDFRendererFormat()
    format.documentInfo = pdfMetaData as [String: Any]
...
//      drawContactLabels(context, pageRect: pageRect, numberTabs: 8)
  ...
  func addBodyText(pageRect: CGRect, textTop: CGFloat) {
    ...
    let attributedText = NSAttributedString(string: body, attributes: textAttributes)
    // 4
    let textRect = CGRect(x: ((pageRect.width - attributedText.size().width) / 2),
                          y: (pageRect.height / 2.5),
  ...
  func addImage(pageRect: CGRect, imageTop: CGFloat) -> CGFloat {
    ...
    let aspectRatio = min(aspectWidth, aspectHeight)
    // 3
    let scaledWidth = image.size.width * aspectRatio / 4
    let scaledHeight = image.size.height * aspectRatio / 4
    // 4
    let imageX = (pageRect.width - scaledWidth) / 2.0
...
class PDFDebugCreator: NSObject {
  let title: String
  let body: String
...
//      drawContactLabels(context, pageRect: pageRect, numberTabs: 8)
  ...
  func addBodyText(pageRect: CGRect) {
    ...
    let attributedText = NSAttributedString(string: body, attributes: textAttributes)
    // 4
    let textRect = CGRect(x: 10, y: 0 + 30, width: pageRect.width - 20,
                          height: pageRect.height - 0 - pageRect.height / 5.0)
    attributedText.draw(in: textRect)
  }
